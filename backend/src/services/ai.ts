...
async function getRagContext(query: string): Promise<string> {
  ...
  try {
    const matches = await db.all(
      `SELECT source_type, content_chunk FROM knowledge_base WHERE ${placeholders} LIMIT 4`,
      params
    );
    ...
    functionDeclarations: [
      ...
      },
      {
        ...
        parameters: {
          type: Type.OBJECT,
          properties: {
            titleOrId: { type: Type.STRING, description: 'Title or ID of the service/scheme.' }
          },
          required: ['titleOrId']
        }
      },
      {
        ...
        parameters: {
          type: Type.OBJECT,
          properties: {
            occupation: { type: Type.STRING, description: 'Citizen occupation (e.g. Farmer, Student, Artisan).' },
            annualIncome: { type: Type.INTEGER, description: 'Family annual income in Rupees.' },
            age: { type: Type.INTEGER, description: 'Citizen age.' },
            state: { type: Type.STRING, description: 'State of residence.' }
          }
        }
      },
      {
        ...
        parameters: {
          type: Type.OBJECT,
          properties: {
            category: { type: Type.STRING, enum: ['roads', 'sanitation', 'water', 'electricity', 'public safety', 'other'], description: 'Category of complaint.' },
            description: { type: Type.STRING, description: 'Detailed description of the issue.' },
            locationText: { type: Type.STRING, description: 'Address or landmark of the issue.' }
          },
      ...
      },
      {
        ...
        parameters: {
          type: Type.OBJECT,
          properties: {
            complaintId: { type: Type.STRING, description: 'Complaint ticket ID (e.g. cmp-001).' }
          },
...
const toolHandlers: Record<string, (args: any) => Promise<any>> = {
  searchServices: async ({ query }) => {
    ...
    const results = await db.all(
      `SELECT id, title, category, description, portal_url, avg_processing_days 
       FROM services WHERE title LIKE ? OR category LIKE ? OR description LIKE ? LIMIT 5`,
      [`%${query}%`, `%${query}%`, `%${query}%`]
    );
    return { results };
  },
  getDocumentChecklist: async ({ titleOrId }) => {
    ...
    const service = await db.get(
      `SELECT title, documents_json FROM services WHERE id = ? OR title LIKE ? LIMIT 1`,
      [titleOrId, `%${titleOrId}%`]
    );
    if (service) {
    ...
    const scheme = await db.get(
      `SELECT title, documents_json FROM schemes WHERE id = ? OR title LIKE ? LIMIT 1`,
      [titleOrId, `%${titleOrId}%`]
    );
    if (scheme) {
  ...
  fileComplaint: async ({ category, description, locationText }) => {
    ...
    await db.run(
      `INSERT INTO complaints (id, citizen_id, category, description, location_text, status, timeline_json) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, citizen_id, category, description, locationText, 'Submitted', JSON.stringify(timeline)]
    );
...
async function generateMockResponse(query: string): Promise<{ text: string; toolCalled?: string; toolResult?: any }> {
...
${res.documents.map((d: string) => `- [ ] ${d}`).join('\n')}
...
Timeline details:
${res.timeline.map((t: any) => `- [${t.status}] ${t.note}`).join('\n')}`,
...
${searchRes.results.map((s: any) => `- **${s.title}** (${s.category}): ${s.description}`).join('\n')}
...
export async function getChatResponse(
  ...
  lang: string = 'en'
): Promise<{ text: string; toolCalled?: string; toolResult?: any }> {
