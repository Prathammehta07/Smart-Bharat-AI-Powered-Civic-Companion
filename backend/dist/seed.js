...
async function seedDatabase() {
    ...
    if (citizenCount.count === 0) {
        ...
        await db.run(`INSERT INTO citizens (id, name, email, phone, language_pref, profile_json) 
       VALUES (?, ?, ?, ?, ?, ?)`, [
            'demo-citizen-123',
            'Rajesh Kumar',
    ...
    if (servicesCount.count === 0) {
        ...
        const stmt = await db.prepare(`INSERT INTO services (id, title, category, description, eligibility_json, documents_json, process_steps_json, portal_url, avg_processing_days) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    ...
    if (schemesCount.count === 0) {
        ...
        const stmt = await db.prepare(`INSERT INTO schemes (id, title, category, description, eligibility_json, benefits, documents_json, target_group_json) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
    ...
    if (complaintsCount.count === 0) {
        ...
        const stmt = await db.prepare(`INSERT INTO complaints (id, citizen_id, category, description, location_text, lat, lng, photo_url, status, created_at, updated_at, timeline_json) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
Target Group: ${target.join(', ')}`;
        ...
        const stmt = await db.prepare(`INSERT INTO knowledge_base (id, source_type, ref_id, content_chunk, keywords) 
       VALUES (?, ?, ?, ?, ?)`);
